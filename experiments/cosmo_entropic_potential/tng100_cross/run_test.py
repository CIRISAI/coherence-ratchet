#!/usr/bin/env python3
"""
TNG100-1 cross-check of the frozen B-total pipeline (75 Mpc/h, independent box).
Mirror of ../large_volume/run_test.py. Executes DECISIONS.md order-of-operations:
  0. k(a) tables -> corner-threshold selection by the FROZEN RULE (counts only, no S)
  1. GPU-estimator validation gate on CAMELS CV_0 (|dS/S| < 1e-6 or abort)
  2. primary S(a) + guards + 8-octant jackknife (GPU Cholesky log-det, box=75)
  3. threshold ladder (7.425e11 = the TNG300 corner, for direct comparison; 5e12)
  4. frozen analysis: w(a), w_today, interior peak (K3), crossing epoch + 68% (P1),
     CPL projections + Mahalanobis (identical machinery), P3/K7 verdicts, mechanism
Incremental writes to results.json after every stage. GPU code copied verbatim from
../large_volume/run_test.py (NOT imported). Only BOX, octant size, ladder, data path,
and the tiled-variant removal differ.
"""
...
BOX = 75.0                                       # TNG100-1
CAP = 38000
LADDER = [7.425e11, 5e12]                         # 7.425e11 = TNG300 corner (direct compare)
OM_PROJ = 0.3155
...
out = {"date": "2026-07-10", "box": "TNG100-1 75 Mpc/h", "cap": CAP,
       "decisions": "DECISIONS.md (pre-committed)",
       "purpose": "independent-box cross-check of the frozen RULE vs TNG300-1"}
def flush():
...
# corner threshold: max over snapshots of the (CAP+1)-th largest mass (the frozen rule:
# lowest threshold keeping every snapshot's count <= CAP). Identical code to TNG300.
...
out["stage0"] = {
    "k_tables": {f"{t:.0e}": [int((sd['m200'] > t).sum()) for sd in snapdata]
                 for t in (1e10, 2e10, 3e10, 5e10, 1e11, 2e11, 5e11, 1e12)},
    "n_kept_per_snap": [int(len(sd["m200"])) for sd in snapdata],
    "thr_corner_Msun_h": float(thr_corner), "k_at_corner": k_corner,
    "k_z3_at_corner": k_z3, "rule_z3_ok": bool(k_z3 >= 200),
    "cap_binds": bool(max(k_corner) >= CAP)}
flush()
log(f"corner threshold = {thr_corner:.3e} Msun/h; k(a) = {k_corner}; "
    f"k(z=3) = {k_z3} (rule >=200: {k_z3 >= 200}); cap binds: {max(k_corner) >= CAP}")
...
# ---------------------------------------------------------------------------
# Stage 1: validation gate on CAMELS CV_0 (CPU op_B vs GPU path) — identical gate
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TNG100 cosmology power spectrum (patch AFTER the gate) — same Planck2015 as TNG300
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 2: primary S(a) + 8-octant jackknife (octants of BOX/2 = 37.5 Mpc/h)
# ---------------------------------------------------------------------------
out["stage2_primary"] = {"thr": float(thr_corner), "records": [], "jackknife": {}}
for sd in snapdata:
...
# ---------------------------------------------------------------------------
# Stage 5: frozen analysis + verdicts (identical machinery)
# ---------------------------------------------------------------------------
...
log(f"  P3/K7: w_today in [-1.22,-0.84]: {-1.22 <= p['w_today'] <= -0.84}")
log(f"  mechanism: k peaks z={out['stage5']['mechanism']['k_peak_z']:.3f}, "
    f"S peaks z={p['interior_peak_z']}")
