#!/usr/bin/env python3
"""
abacus_cross science phase (restart): corner-equivalent + full-population at T=4 and T=8 only.
Gate v2 already PASSED and saved (results.json gate_v2). T=2 dropped for the science configs:
at the 500-box corner/full populations a T=2 tile holds ~87k-290k halos, exact-det-infeasible
and subsample-contaminated (not the clean estimator); the gate validated the T=8 62.5-Mpc/h
grain, and TNG showed tile-robustness. Verdict is read at T=8 (the gated grain). Per-snapshot
log + flush. Preserves the already-computed T=2 corner record (flagged subsample-contaminated).
"""
...
def tiled_S(pos, T, seed0, logf=None):
    side = BOX / T
...
log(f"loaded {len(snapdata)} snaps; LCDM chi2={LCDM_CHI2:.4f}; gate v2 PASS preserved")
...
def run_config(thr, key):
    rec = dict(out.get(key, {}))          # preserve any already-computed tiles (e.g. T=2)
    for T in TILES:
        Stot, Sint, per = [], [], []
        for si, sd in enumerate(snapdata):
            ...
            rec[str(T)] = {"per_snap": per}; out[key] = rec; flush()
            log(f"    [{key} T={T}] a={sd['a']:.3f} n={len(pos)} S={r['S_total']:.1f} "
                f"occ[{r['occ_min']},{r['occ_max']}] flagged={r['n_flagged']}")
...
log("=== CORNER-EQUIVALENT 7.425e11 (T=4,8) ===")
out["corner_7.425e11"] = run_config(CORNER, "corner_7.425e11"); flush()
log("=== FULL POPULATION 2.109e11 (T=4,8) ===")
...
flush()
log(f"GRID CAVEAT: TNG full ({full['w0']:.3f},{full['wa']:.3f},cross={full['cross_z']}) -> "
    f"Abacus-grid ({regrid['w0']:.3f},{regrid['wa']:.3f},cross={regrid['cross_z']}) "
    f"Dw0={out['grid_caveat']['delta']['w0']:+.3f} Dwa={out['grid_caveat']['delta']['wa']:+.3f}")

# ---- verdict: corner-equivalent, primary tile T=8 (gated grain), S_total ----
BEST_T = 8
...
def verdict(a):
    ...
    return "LAW-LIKE" if n == 3 else ("CODE-SPECIFIC" if n == 0 else "PARTIAL")
vw = verdict(v); vfull = verdict(out["full_2.109e11"][str(BEST_T)]["S_total_analysis"])
comp = {"interior_peak": bool(v["z_peak"] is not None and 0.3 <= v["z_peak"] <= 1.0 and not v["peak_at_endpoint"]),
...
out["VERDICT"] = {"word": vw, "gate": "v2 (fair-regime subvolume) PASS", "best_tile": BEST_T,
                  ...
                  "thawing": v["thawing"], "maha": v["maha"], "beats_lcdm": comp["beats_lcdm"],
                  "chi2_minus_lcdm": v["chi2_minus_lcdm"], "components": comp,
                  "n_corner": out["corner_7.425e11"][str(BEST_T)]["per_snap"][-1]["n"],
                  "full_population_companion_verdict": vfull,
                  "full_population_components": {
                      "z_peak": out["full_2.109e11"][str(BEST_T)]["S_total_analysis"]["z_peak"],
                      "wa": out["full_2.109e11"][str(BEST_T)]["S_total_analysis"]["wa"],
                      "maha": out["full_2.109e11"][str(BEST_T)]["S_total_analysis"]["maha"]}}
flush()
log("=" * 70)
log(f"VERDICT: {vw}  corner T={BEST_T} z_peak={v['z_peak']:.3f} w0={v['w0']:.3f} wa={v['wa']:.3f} "
    f"thaw={v['thawing']} maha={v['maha']:.3f} beats_lcdm={comp['beats_lcdm']} | full-pop: {vfull}")
log("DONE"); flush()
