...
print()
print("=" * 78)
print("FRAMEWORK-DISTINCTIVE LAYER -- what is NOT shared with generic attractors")
print("=" * 78)
print("  Signed drift alone (low-ell down, high-ell up) follows for ANY")
print("  corridor-attractor whose centre lies between the low-ell and high-ell")
print("  observed values. It is necessary, not sufficient. The framework-")
print("  distinctive content is the per-ell centre CURVE and the crossover.")
print()
KEFF_MID = 0.5 * (KEFF_LO + KEFF_HI)
RHO_FLAT = 0.25                              # single-rho_mid attractor null
centre = {ell: rho_at(2 * ell + 1, KEFF_MID) for ell in ELLS}
print(f"  {'ell':>4}{'framework centre':>18}{'flat null':>11}{'observed':>10}")
for ell in [2, 3, 4, 5, 6, 8, 12, 20, 30]:
    print(f"  {ell:>4}{centre[ell]:>18.3f}{RHO_FLAT:>11.2f}{obs[ell]:>10.3f}")


def crossover(centre_of):
    for ell in ELLS:
        if obs[ell] < centre_of(ell):
            return ell
    return None


fw_x = crossover(lambda l: centre[l])
flat_x = crossover(lambda l: RHO_FLAT)
print()
print(f"  framework per-ell centre: RISES {centre[ELLS[0]]:.2f} -> "
      f"{centre[ELLS[-1]]:.2f} (Mobius, asymptote 1/k_eff); the observed")
print(f"    profile crosses it at ell = {fw_x}.")
print(f"  single-rho_mid attractor: FLAT centre; crossover ell = {flat_x} -- but")
print(f"    rho_mid is a FREE parameter, it FITS the crossover. The framework")
print(f"    FIXES the whole centre curve from A3+ calibration, out-of-sample:")
print(f"    no CMB-fitted parameter. The distinctive test is the SHAPE of the")
print(f"    per-ell attractor (rising Mobius vs flat) and that crossover")
print(f"    ell ~ {fw_x} is PREDICTED, not fitted.")
print()
print("=" * 78)
print("VERDICT -- held at full rigor")
print("=" * 78)
print("  The framework predicts, at the CMB substrate:")
print("  (a) SIGNED drift at each ell -- a real prediction, but partially")
print("      SHARED with any generic corridor-attractor theory. Necessary,")
print("      not sufficient: a test against any corridor theory, not this one.")
print("  (b) PER-ELL corridor centres via A3+ calibration + Kish k-translation")
print("      -- a rising Mobius curve, FRAMEWORK-DISTINCTIVE (single-attractor")
print("      theories have a flat centre).")
print(f"  (c) CROSSOVER ell ~ {fw_x} between drift-down and drift-up, FIXED by")
print("      out-of-sample calibration -- framework-distinctive (a generic")
print("      attractor FITS its crossover; the framework PREDICTS it).")
print(f"  (d) drift MAGNITUDE band-limited to ~{np.mean(widths):.2f} at current")
print("      calibration -- calibration work, not yet a prediction;")
print("      tightenable via the per-substrate effective-k measurement.")
print()
print("  (a) tests against ANY corridor theory; (b)+(c) are what CMB-S4 would")
print("  test against THIS framework specifically; (d) is calibration. This is")
print("  resolution (2) for the framework-distinctive content -- which lives in")
print("  the per-ell centres and the crossover, not in the bare signed drift.")
print("  No new parameter, no curve-fit, out-of-sample. Named at full rigor.")
